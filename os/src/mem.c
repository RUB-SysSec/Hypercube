...
#ifdef MEM_DEBUG
	debug_printf(STR_PREFIX" allocating ~%d KB (nframes)\n\r", (INDEX_FROM_BIT(nframes * 8) >> 10));
#endif
...
static void paging_install() {
	...
	/* remap payload buffer somewhere else */
	for (uintptr_t i = (uintptr_t)&kernel_end; i < (uintptr_t)&runtime_data_end; i += 0x1000) {
		dma_frame(get_page((i + PAYLOAD_BUFFER_VIRTUAL_OFFSET) & 0xFFFFF000, 1, kernel_directory, true), 1, 1, i & 0xFFFFF000);
	}
...
#ifdef MEM_DEBUG
	debug_printf(STR_PREFIX " new cr3 value is %x\n\r", kernel_directory);
#endif
	paging_enabled = 1;
}
...
uintptr_t mem_alloc_ap_stack(void){
	uintptr_t ptr;
#ifdef MEM_DEBUG
	debug_printf("Try to alloc\n\r");
#endif
	ptr = (uintptr_t) kmalloc(0x1000);
#ifdef MEM_DEBUG
	debug_printf(STR_PREFIX "Done: %x", ptr);
#endif
...
void print_mem_stats(void){
	debug_printf("\n\r-------------------------\n\r");
	debug_printf(STR_PREFIX " MEMSTATS:\n\r");
	debug_printf(STR_PREFIX " kmalloc: %d\n\r", counter_kmalloc);
	debug_printf(STR_PREFIX " kvmalloc: %d\n\r", counter_kvmalloc);
	debug_printf(STR_PREFIX " kmalloc_p: %d\n\r", counter_kmalloc_p);
	debug_printf(STR_PREFIX " kvmalloc_p: %d\n\r", counter_kvmalloc_p);
	debug_printf("\n\r");
	debug_printf(STR_PREFIX " pmalloc: %d\n\r", counter_pmalloc);
	debug_printf(STR_PREFIX " pvmalloc: %d\n\r", counter_pvmalloc);
...
bool mem_init(void){
	...
	debug_printf (STR_PREFIX" setup memory management...\n\r");
	debug_printf(STR_PREFIX " Low:  %x\n\r", low_mem * 0x400);
	debug_printf(STR_PREFIX " High: %x\n\r", high_mem * 0x400);
	debug_printf(STR_PREFIX " End:  %x\n\r", &kernel_end);
	debug_printf(STR_PREFIX " Total:  %d MB\n\r", (low_mem+high_mem)/0x400);
	...
	debug_printf (STR_PREFIX" enabling paging...\n\r");
	if(efi_mmap){
	...
	if(!mmap_configured){
		debug_printf (STR_PREFIX" mmap not found...\n\r");
		//return false;
	}
