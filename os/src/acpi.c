...
static void acpi_apic_type_0_parse(uintptr_t apic_ptr_offset){
	...
	debug_printf(STR_PREFIX"               processor_id %1x\n\r", data->processor_id);
	debug_printf(STR_PREFIX"               apic_id %1x\n\r", data->apic_id);
...
static void acpi_apic_type_1_parse(uintptr_t apic_ptr_offset){
	...
	debug_printf(STR_PREFIX"               io_apic_id %1x\n\r", data->io_apic_id);
	debug_printf(STR_PREFIX"               io_apic_address %4x\n\r", data->io_apic_address);
	debug_printf(STR_PREFIX"               global_sys_intr_base %4x\n\r", data->global_sys_intr_base);
...
static void acpi_apic_type_2_parse(uintptr_t apic_ptr_offset){
	...
	debug_printf(STR_PREFIX"               bus %1x\n\r", data->bus);
	debug_printf(STR_PREFIX"               source %1x\n\r", data->source);
	debug_printf(STR_PREFIX"               global_sys_intr %4x\n\r", data->global_sys_intr);
	debug_printf(STR_PREFIX"               flags %2x\n\r", data->flags);
...
static void acpi_apic_type_4_parse(uintptr_t apic_ptr_offset){
	...
	debug_printf(STR_PREFIX"               acpi_processor_id %1x\n\r", data->acpi_processor_id);
	debug_printf(STR_PREFIX"               flags %2x\n\r", data->flags);
	debug_printf(STR_PREFIX"               lapic_lint %1x\n\r", data->lapic_lint);
}
...
static void acpi_apic_parse(uintptr_t apic_ptr){
	...
	if (!(checksum_value & 0xff)){
		...
		debug_printf(STR_PREFIX"         APIC lapic_address: %x\n\r", data->lapic_address);
		debug_printf(STR_PREFIX"         APIC flags: %x\n\r", data->flags);

		debug_printf(STR_PREFIX"         APIC len: %d\n\r", data->acpi.len);
		debug_printf(STR_PREFIX"         APIC oem: ");
		for(uint8_t i = 0; i < 6; i++){
			debug_printf("%c", data->acpi.oem[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"         APIC oem_table: ");
		for(uint8_t i = 0; i < 8; i++){
			debug_printf("%c", data->acpi.oem_table[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"         APIC oem_rev: %x\n\r", data->acpi.oem_rev);
		debug_printf(STR_PREFIX"         APIC creator_id: %x\n\r", data->acpi.creator_id);
		debug_printf(STR_PREFIX"         APIC creator_rev: %x\n\r", data->acpi.creator_rev);

		for(uint16_t i = 44; i < data->acpi.len; i += *((uint8_t*)(apic_ptr+i+1))){
			type = *((uint8_t*)(apic_ptr+i));
			len = *((uint8_t*)(apic_ptr+(i+1)));
			debug_printf(STR_PREFIX"            type: %d len: %d\n\r", type, len);
			switch(type){
...
static void acpi_facp_parse(uintptr_t facp_ptr){
	...
	debug_printf(STR_PREFIX"         FACP len:		  %x\n\r", data->len);
	debug_printf(STR_PREFIX"         FACP dsdt_ptr:	  %x\n\r", data->dsdt_ptr);
	//hexdump("DSDT:", data->dsdt_ptr, 0x1000);

	for(uintptr_t i = (uintptr_t)data->dsdt_ptr; i < (uintptr_t)data->dsdt_ptr+0x1000; i++){
		if (*((uint32_t*)i) == 0x5f35535f ){
			debug_printf(STR_PREFIX"         Found \"_S5_\" at %x!\n\r", i-(uintptr_t)(data->dsdt_ptr));

			break;
		}
	}

	debug_printf(STR_PREFIX"         FACP SMI_CMD:	  %x\n\r", data->SMI_CMD);
	debug_printf(STR_PREFIX"         FACP ACPI_ENABLE:  %x\n\r", data->ACPI_ENABLE);
	debug_printf(STR_PREFIX"         FACP ACPI_DISABLE: %x\n\r", data->ACPI_DISABLE);

	debug_printf(STR_PREFIX"         FACP PM1a_CNT_BLK: %x\n\r", data->PM1a_CNT_BLK);
	debug_printf(STR_PREFIX"         FACP PM1b_CNT_BLK: %x\n\r", data->PM1b_CNT_BLK);
	debug_printf(STR_PREFIX"         FACP PM1_CNT_LEN:  %x\n\r", data->PM1_CNT_LEN);
}
...
static void acpi_mcfg_parse(uintptr_t mcfg_ptr){
	...
	if (!(checksum_value & 0xff)){
		...
		debug_printf(STR_PREFIX"         MCFG len: %d [Entries: %d]\n\r", data->acpi.len, entries);
		debug_printf(STR_PREFIX"         MCFG oem: ");
		for(uint8_t i = 0; i < 6; i++){
			debug_printf("%c", data->acpi.oem[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"         MCFG oem_table: ");
		for(uint8_t i = 0; i < 8; i++){
			debug_printf("%c", data->acpi.oem_table[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"         MCFG oem_rev: %x\n\r", data->acpi.oem_rev);
		debug_printf(STR_PREFIX"         MCFG creator_id: %x\n\r", data->acpi.creator_id);
		debug_printf(STR_PREFIX"         MCFG creator_rev: %x\n\r", data->acpi.creator_rev);
		...
		for(uint16_t i = 0; i < entries; i++){
			...
			debug_printf(STR_PREFIX"            MCFG%d bar: 0x%4x%4x\n\r", i, mcfg_data->bar_ecam_hi, mcfg_data->bar_ecam_lo);
			debug_printf(STR_PREFIX"            MCFG%d pci_seg_group: 0x%2x\n\r", i, mcfg_data->pci_seg_group);
			debug_printf(STR_PREFIX"            MCFG%d start: 0x%1x\n\r", i, mcfg_data->pci_start);
			debug_printf(STR_PREFIX"            MCFG%d end:   0x%1x\n\r", i, mcfg_data->pci_end);
		}
	}
}
...
static void acpi_rsdt_parse(uintptr_t rsdt_ptr){
	...
	if (!(checksum_value & 0xff)){
		debug_printf(STR_PREFIX"      RSDT len: %d [Entries: %d]\n\r", data->len, (data->len-sizeof(struct rsdt_desc)) / 4);
		debug_printf(STR_PREFIX"      RSDT oem: ");
		for(uint8_t i = 0; i < 6; i++){
			debug_printf("%c", data->oem[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"      RSDT oem_table: ");
		for(uint8_t i = 0; i < 8; i++){
			debug_printf("%c", data->oem_table[i]);
		}
		debug_printf("\n\r");
		debug_printf(STR_PREFIX"      RSDT oem_rev: %x\n\r", data->oem_rev);
		debug_printf(STR_PREFIX"      RSDT creator_id: %x\n\r", data->creator_id);
		debug_printf(STR_PREFIX"      RSDT creator_rev: %x\n\r", data->creator_rev);

		for(uint32_t i = 0; i < (data->len-sizeof(struct rsdt_desc)) / 4; i++){
			debug_printf(STR_PREFIX"      -> 0x%x ", ((uint32_t*)(rsdt_ptr + sizeof(struct rsdt_desc)))[i]);
...
static void acpi_rsdp_parse(uintptr_t rsd_ptr){
	...
	if (!(checksum_value & 0xff)){
		debug_printf(STR_PREFIX"   RSDP oem: ");
		for(uint8_t i = 0; i < 6; i++){
			debug_printf("%c", data->oem[i]);
		}
		debug_printf("\n\r");
		switch(data->rev){
			case 0:		debug_printf(STR_PREFIX"   RSDP rev: ACPI 1.0\n\r"); break;
			case 2:		debug_printf(STR_PREFIX"   RSDP rev: ACPI 2.0\n\r"); break;
			default:	debug_printf(STR_PREFIX"   RSDP rev: %2x\n\r", data->rev); break;
		}
		debug_printf(STR_PREFIX"   RSDP ptr: %x\n\r", data->rsdt_ptr);
...
static bool acpi_init_bios(void){
	for(uint32_t i = 0x000A0000; i < 0x000FFFFF; i+=8){
		if(RSD_PTR_STR == *((uint64_t*)i)){ /* "RSD PTR " */
			debug_printf(STR_PREFIX" RSDP: 0x%x\n\r", i);
...
void acpi_init(void){
	if(acpi_ptr){
		debug_printf(STR_PREFIX" using multiboot2 rsdp ptr...\n\r");
	...
	debug_printf(STR_PREFIX" not supported...\n\r");
}
