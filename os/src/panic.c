...
void panic_handler(struct regs* r) {
	...
	/* recover from GPF / Double Fault causd by rdmsr / wrmsr */
	if(r->int_no == 13 || r->int_no == 8){
		uint16_t* ins = (uint16_t*)r->eip;
		if(*ins == RDMSR_INS || *ins == WRMSR_INS){
				//printf("GPF ");
				r->eip += 2;
				return;
		}
...
#ifdef PANIC_AUTO_REBOOT
  reboot2();
#endif
    
	terminal_initialize(CRASH_COLOR);
	printf("\n\rKernel Panic :-(\n\r\n\r");
