...
void smp_init(void){
	...
	debug_printf(STR_PREFIX" Starting %d APs...\n", aps);
	...
	for(uint32_t i = 1; i < aps; i++){
		...
		if ((mmio_read32(base, LAPIC_ICRLO) & ICR_SEND_PENDING) != 0) {
			debug_printf(STR_PREFIX" AP-WAKENING FAIL!\n");
		}
		...
		debug_printf(STR_PREFIX" Processor %d ready...\n", ap_ready);
	}
	debug_printf(STR_PREFIX" Processor 0 ready...\n");
}
