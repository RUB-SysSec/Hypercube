...
static void io_apic_init(void){
	if (!ioapic_base){
		debug_printf(STR_PREFIX" IOAPIC base not found ...\n\r");
		return;
	}

	debug_printf(STR_PREFIX" disabling PIC!\n\r");
	pic_disable();
	asm volatile("cli\n\r");
    ...
    debug_printf(STR_PREFIX" I/O APIC pins = %d\n\r", count);
...
bool apic_init(void){
	...
	if(apic_check_supported()){
		debug_printf(STR_PREFIX" supported!\n\r");

		base = apic_read_msr_base();
		base = apic_enable(base);
		apic_set_base(base);
		debug_printf(STR_PREFIX" base is 0x%x\n\r", base);
		...
		debug_printf(STR_PREFIX" LAPIC ID: %2x\n\r", apic_get_local_id());
