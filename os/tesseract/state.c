...
#ifdef HYPERCUBE
		//debug_printf(STR_PREFIX"Scratch area #%d located at 0x%x\n", i, state_obj->alloc_areas[i]);
#endif
...
void print_state(state_t* state){
	...
	for(uint16_t i = 0; i < state->num_mmio_areas; i++){
		printf(STR_PREFIX"registered area - base:0x%x size:0x%x MMIO -> \"%s\"\n\r", state->mmio_area[i]->base, state->mmio_area[i]->size, state->mmio_area[i]->desc);

		//printf("mmio_area[%d] =\t{\n\r\t\t  base = %0x;\n\r\t\t  size = 0x%x;\n\r\t\t  desc = \"%s\";\n\r\t\t};\n\r", i, state->mmio_area[i]->base, state->mmio_area[i]->size, state->mmio_area[i]->desc);
	}
	for(uint16_t i = 0; i < state->num_io_areas; i++){
		printf(STR_PREFIX"registered area - base:0x%x size:0x%x PIO  -> \"%s\"\n\r", state->io_area[i]->base, state->io_area[i]->size, state->io_area[i]->desc);
